...
class MacHotkeyImpl:
    def __init__(self, config, queue):
        ...
        for action, combo in config.items():
            pc = _to_pynput(combo)
            if pc is None:
                print(f"macos hotkey: skipping {action}: unsupported key in '{combo}'",
                      file=sys.stderr)
            else:
    ...
    def start(self):
        ...
        try:
            ...
            print("macos hotkey: pynput not available (pip install pynput)",
                  file=sys.stderr)
            return
        ...
        try:
            ...
            print(f"macos hotkey: failed to start: {e}", file=sys.stderr)
